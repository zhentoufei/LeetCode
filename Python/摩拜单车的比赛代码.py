...
__date__ = '2017/10/24 22:56'
__site__ = ''
__software__ = 'PyCharm'
__file__ = '摩拜单车的比赛代码.py'
...
cache_path = os.path.abspath('mobike_cache1/')
train_path = os.path.abspath('data/temp_train.csv')
test_path = os.path.abspath('data/temp_test.csv')
flag = True
...
if __name__ == "__main__":
    ...
    train = pd.read_csv(train_path)
    test = pd.read_csv(test_path)
    train1 = train[(train['starttime'] < '2017-05-16 00:00:00')]
    train2 = train[(train['starttime']>= '2017-05-16 00:00:00')]
    ...
    gbdt = GradientBoostingClassifier(subsample=0.886,max_depth=10)
    gbdt.fit(train_feat[predictors], train_feat['label'])
    train_feat.loc[:,'pred_gbdt']=gbdt.predict(train_feat[predictors])
    #xgb训练结果
    xgbtrain = xgb.DMatrix(train_feat[predictors], train_feat['label'])
    xgbtest = xgb.DMatrix(test_feat[predictors])
    xgb = xgb.train(params, xgbtrain, num_boost_round=120)
    train_feat.loc[:,'pred_xgb']=xgb.predict(train_feat[predictors])

    #blending
    from sklearn.linear_model import LogisticRegression
    LR = LogisticRegression()  # 使用类，参数全是默认的
    LR.fit(train_feat['pred_xgb','pred_gbdt'], train_feat['label'])  # 训练数据来学习，不需要返回值

    test_feat.loc[:,'pred_xgb'] = xgb.predict(xgbtest)
    test_feat.loc[:, 'pred_gbdt'] = gbdt.predict(xgbtest)
    test_feat[:,'pred']=LR.predict(test_feat['pred_xgb','pred_gbdt'])
